...
import com.evivv.store.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public User login(String username, String password) {
        // 1. 通过传递过来的username，并在数据库中查询是否存在
        User result = getOne(new QueryWrapper<User>().eq("username", username));
        // 判断结果集为null，则抛出异常
        if (result == null) {
    ...
    public void register(User user) {
        ...
        User result = getOne(new QueryWrapper<User>().eq("username", user.getUsername()));
